...
// Use a for Loop through the extracted books and create an element for each
...
showMore.innerHTML = /* html */`
  <span>Show more</span>,
  <span class="list__remaining">(${matches.length - BOOKS_PER_PAGE > 0 ? books.length : 0})</span>`;
showMore.disabled = !(books.length - BOOKS_PER_PAGE > 0);
...
//Disable the button if there are no more books to show
 showMore,disabled = !(books.length - endIndex > 0);
});
...
menu.addEventListener('click', information);
...
const saveBtn = document.querySelector("body > dialog:nth-child(5) > div > div > button.overlay_button.overlay_button_primary")
